...
function renderSegments(el, content) {
  ...
  for (const seg of segs) {
    ...
    if (typeof seg === 'string') {
      ...
      // (what every shipped cross-link uses), OR the nested { link: { href, text } }
      // that an early AUTHORING example showed. The nested form used to fall through
      // every branch and render NOTHING -- a note link that silently failed to
      // appear at all. Accepting both means a mis-shaped link can never vanish
      // without a trace again; flat stays canonical (see AUTHORING "Links").
