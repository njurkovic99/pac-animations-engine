 ...
 *               code has), 'twoThirdsStage' (≤ two-thirds of the stage), or a COUNT {frames:N}/{rows:N}
...
const SIZE_POLICY = {
  ...
  structure: { floorRows: 2,  ceiling: 'twoThirdsStage', scroll: true, width: 'natural' }, // ≤ two-thirds of the stage (v-scroll ok); natural width, never h-scrolls. Raised from ½: the ½ cap was set speculatively before any animation had a real tree; trees-bst-delete (12 nodes, 5 levels) is the first, and panning a structure the student is reading is worse than a slightly taller stage.
...
export class Engine {
   ...
   *      takes what it needs up to a CEILING of two-thirds the stage height, its panes
  ...
  layoutStage() {
    ...
    // Horizontal room the code panel keeps to the RIGHT of its widest line, so a long
    // listing's vertical scrollbar never overlaps that line and forces a sideways
    // scroll. This is the slack the old "code fills the column" behaviour provided for
    // free and the earlier bare "size to content" rule dropped -- reserving it
    // explicitly is what lets the panel hug its widest line without that regression.
    // A classic scrollbar is ~17px; the CI/headless overlay scrollbar is 0px, so this
    // reserve is invisible to the harness and must be a constant, not a measurement.
    const SCROLLBAR = 17;
    ...
    let   book      = items.filter(p => p.kind === 'book');

    // ARROW-ENDPOINT ADJACENCY (AUTHORING.md "Arrows keep their endpoints
    // adjacent"). A cross-panel arrow is DATA -- the overlay draws it against live
    // boxes but has no say in WHERE panels sit, so two panels wired together can end
    // up with a third BETWEEN them, and the arrow then has to cross it. The arrow
    // set is known from the trace before layout resolves, so the panels it
    // references form a GROUP kept contiguous: the arrow-endpoint BOOK panels are
    // ordered FIRST (directly under the structure region, itself usually an
    // endpoint), non-endpoints after, and the column split below is floored (kMin)
    // so no non-endpoint book panel lands between the region and an endpoint book
    // panel. A panel that is not an arrow endpoint never sits between two that are.
    const arrowPanels = new Set();
    for (const s of this.steps) for (const a of (s.arrows ?? [])) {
      const fp = String(a.from).split('.')[0], tp = String(a.to).split('.')[0];
      if (fp && tp && fp !== tp) { arrowPanels.add(fp); arrowPanels.add(tp); }
    }
    const isEndpoint = p => arrowPanels.has(p.spec.id ?? p.spec.type);
    const structIsEndpoint = structure.some(isEndpoint);
    const groupBook = book.filter(isEndpoint);
    // Reorder only when arrows tie a book panel to the structure region -- the case
    // that can trap a non-endpoint between them. Endpoint book panels move to the
    // front (each subset keeps its declared order), so they sit adjacent to the
    // region; the split floor then keeps them there.
    if (structIsEndpoint && groupBook.length && groupBook.length < book.length) {
      book = [...groupBook, ...book.filter(p => !isEndpoint(p))];
    }
    // Every endpoint book panel must stay in the RIGHT column, contiguous with the
    // structure region (which holds the other endpoint): k below this floor would
    // drop one to the left column, splitting the group across the divide so its
    // arrow crosses the code panel to reach it.
    const kMin = structIsEndpoint ? groupBook.length : 0;
    ...
    // Code's CONTENT width: its widest line plus a one-character allowance so the last
    // glyph never touches the border. This is the basis of the FLOOR (codeMinW below);
    // the scrollbar reserve is deliberately NOT folded in here, so a borderline ~60-char
    // listing is not nudged past the minCh floor and made wider than it was.
    const codeContentW = code ? Math.round(codeWidestW + Math.ceil(codeCharW)) : 0;
    // Code's CEILING width (item "code needs a width ceiling"): the content width plus a
    // SCROLLBAR reserve, so a long listing's vertical scrollbar sits to the RIGHT of the
    // widest line rather than over it. This is the MAX the panel ever takes -- like the
    // 'listing' height ceiling caps its rows, this caps its columns, so on a wide stage
    // the panel is exactly as wide as its longest line (plus that reserve) and the
    // leftover column width stays EMPTY to its right (not distributed, not handed to the
    // structure -- that would undo item 39). The reserve is why capping here does NOT
    // reintroduce the sideways-scroll the bare "size to content" rule caused: the widest
    // line always clears the scrollbar.
    const codeCeilW = code ? codeContentW + SCROLLBAR : 0;
    ...
    // (SIZE_POLICY.structure = 'twoThirdsStage'), scrolling beyond. Values from the table.
    const ceiling = SIZE_POLICY.structure.ceiling === 'twoThirdsStage'
      ? Math.max(140, Math.floor(H * 2 / 3))
      : SIZE_POLICY.structure.ceiling === 'halfStage'
    ...
    // k starts at kMin so arrow-endpoint book panels never fall to the left column
    // (see ARROW-ENDPOINT ADJACENCY above); the balancer then picks the shortest
    // taller column among the splits that keep the group contiguous.
    let bestK = kMin, bestStage = Infinity, bestGap = Infinity;
    for (let k = kMin; k <= book.length; k++) {
    ...
    if (singleCol) {
      ...
      for (const p of this.panels.values()) {
        if (p.kind === 'code') {
          blocks.push({ el: code.el, w: codeCeilW, h: codeSingleH, floor: codeFloor, kind: 'code' });
      ...
      for (const row of rows) for (const blk of row.blocks) {
        if (blk.kind === 'code') {
          ...
          code.el.style.width = `${Math.round(codeCeilW)}px`;
   ...
   *  a structure's SVG/boxes (whose twoThirdsStage ceiling is applied later in paneH),
