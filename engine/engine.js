...
export class Engine {
  ...
  layoutStage() {
    ...
    // EVERY bookkeeping panel gets a FIXED height with flex:none, derived from its
    // own content (p.contentH, resolved once in _panelContentH). flex:none is not
    // optional: the CSS default is `.pac-panel-body { flex: 1 }`, and a body left
    // with that flex-grow will STRETCH to fill whatever column space is below it --
    // the single mechanism behind every "panel grew instead of sizing to content"
    // regression. Pinning flex:none on all of them removes the hazard in one place;
    // the height each gets is what _panelContentH computed (the stream's emitted
    // row count, the callstack's capped frame count, a strip's measured rows).
    for (const p of book) {
      const body = p.el.querySelector('.pac-panel-body');
  ...
  /** The single place a panel's content height is resolved -- every non-code panel
   *  (structure and bookkeeping) goes through here, so "size to your own content"
   *  lives in ONE function instead of being re-derived per panel. Each kind is
   *  derived from an integer COUNT of its own rows, not a raw pixel, the same
   *  content-derived calc the code panel uses (rows x line-height + padding):
   *   - STREAM: its EMITTED line count. The trace fixes it (fib emits 18), so the
   *     panel is exactly that many rows, from step 0, and never grows.
   *   - CALLSTACK: its frame count, capped at CALLSTACK_CEILING (a deep stack has no
   *     natural bound, so past the ceiling it scrolls).
   *   - CELLS / NODES (a strip, an array, a tree): irregular content (cell boxes, an
   *     SVG), so their genuine rendered height IS the content -- scrollHeight. This
   *     is content, not stretch: the body is pinned flex:none, so nothing inflates
   *     it beyond what it renders.
   *  (The code panel resolves its own height in the fill block, from its line count
   *  and the column slack it alone absorbs -- the reference implementation.) */
  _panelContentH(p, body) {
    const cs = getComputedStyle(body);
    const pad = (parseFloat(cs.paddingTop) || 11) + (parseFloat(cs.paddingBottom) || 11);
    if (p.spec.type === 'stream') {
      const lines = [...body.querySelectorAll('.pac-stream-line')];
      if (!lines.length) return body.scrollHeight;              // step 0: nothing emitted
      const lineH = lines[0].getBoundingClientRect().height || 21;
      return Math.ceil(lines.length * lineH) + pad;              // rows x line-height + padding
    }
    if (p.spec.type === 'callstack') {
      const frames = [...body.querySelectorAll('.pac-frame')];
      if (frames.length > CALLSTACK_CEILING) {
        const top = body.getBoundingClientRect().top;
        return Math.round(frames[CALLSTACK_CEILING - 1].getBoundingClientRect().bottom - top
          + (parseFloat(cs.paddingBottom) || 11));
      }
    }
