 ...
 * read as "however many that is" where a handful did not; each keeps the clear
 * 5px line / 4px gap spacing that made the lines legibly SEPARATE at 1x (a tighter
 * pack smears into fewer lines at real viewing scale). The lines block, and so
 * BUNDLE_H, is taller than with five lines — but the PANEL's dimensions (the SVG's
 * fixed W/H) do not depend on BUNDLE_H and do not change, and the taller bundle
