 ...
 *               has), 'halfStage' (≤ half the stage), or {frames:N}
 *   scroll    : true if content past the ceiling scrolls rather than being cut
 * Adding a panel type is a NEW ROW here, not a new branch. See _sizeClass, which
 * maps a panel to its class, and _panelContentH / the height phase, which apply
 * these uniformly. (AUTHORING.md "Panel sizing policy".) */
const SIZE_POLICY = {
  ...
  stream:    { floorRows: 2,  ceiling: null                      },  // exactly its emitted lines
...
export class Engine {
   ...
   *   - CALLSTACK: its frame count, capped at SIZE_POLICY.callstack's frame ceiling
   *     (a deep stack has no natural bound, so past the ceiling it scrolls).
  ...
  _panelContentH(p, body) {
    ...
    if (p.spec.type === 'callstack') {
      const cap = SIZE_POLICY.callstack.ceiling.frames;           // from the policy table
      const frames = [...body.querySelectorAll('.pac-frame')];
      if (frames.length > cap) {
        const top = body.getBoundingClientRect().top;
        return Math.round(frames[cap - 1].getBoundingClientRect().bottom - top
