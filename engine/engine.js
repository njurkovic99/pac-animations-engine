 ...
 *                 'fill'    : takes the width left after the natural panels, down to
 *                             `minCh` characters (code)
...
const SIZE_POLICY = {
  code:      { floorRows: 15, ceiling: 'listing',     width: 'fill', minCh: 60 }, // ≥15 rows; width = remainder, ≥60 chars
...
export class Engine {
  ...
  layoutStage() {
    ...
    // Code's minimum width: enough to show its widest line uncompressed, but never
    // DEMANDING more than `minCh` characters -- a listing with short lines needs
    // only its own width (so the structure gets the rest); a listing with lines
    // longer than minCh is held at minCh and scrolls, because the structure has
    // width priority. min(widest line, minCh chars + gutter).
    const codeMinCh = code ? SIZE_POLICY.code.minCh * codeCharW + codeGutterW + bodyPad + BORDER : 0;
    const codeMinW  = code ? Math.round(Math.min(codeWidestW, codeMinCh)) : 0;
    ...
    // 'column' (stream) fills its column via CSS. 'fill' (code) fills the rest via
    // CSS flex, held at >= codeMinW by an inline min-width so it never drops below
    // its char floor. ----
