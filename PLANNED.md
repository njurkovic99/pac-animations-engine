...
## Emit machinery for `?a=` markers

The hidden assignment marker (`...html?a=ds-a2`) is inert and instructor-only, and
the engine correctly ignores it. Two deliverables are deferred:

- emitting `?a=` automatically when generating per-course iframe URLs from
  `courses.json`
