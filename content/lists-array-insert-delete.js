 ...
 * INSERT(7, 99) [the invalid one]; DELETE(1) — so every call has a visible
 * origin. The CALLSTACK panel is a debugger's locals/stack pane: frames push on
 * call (newest on top, showing their BOUND parameter values) and pop on return
 * (the caller below is greyed but present). This makes the ADD-is-INSERT lesson
 * visible: ADD(40) calls INSERT(count, value), and INSERT's frame appears already
 * showing index = 3 (count's value) and value = 40 — the parameters you could
 * previously only infer from narration.
 *
 ...
 * before making room; inserting out of range) are taught with NARRATION and
 ...
 * index is a logic/contract error, not memory corruption. Argument→parameter
 * flow animation and pass-by-value/reference are deferred (AUTHORING.md
...
const LISTINGS = {
  pseudo: [
    ...
    '',                                              // 6
    'ADD(value):',                                   // 7
    '    INSERT(count, value)',                       // 8
    '',                                              // 9
    'INSERT(index, value):',                          // 10
    '    if index < 0 or index > count:',             // 11
    '        report "invalid index"; return',         // 12
    '    for i = count down to index + 1:',           // 13
    '        A[i] = A[i - 1]',                         // 14
    '    A[index] = value',                            // 15
    '    count = count + 1',                           // 16
    '',                                              // 17
    'DELETE(index):',                                 // 18
    '    if index < 0 or index >= count:',            // 19
    '        report "invalid index"; return',         // 20
    '    for i = index to count - 2:',                // 21
    '        A[i] = A[i + 1]',                         // 22
    '    count = count - 1',                           // 23
  ],
  java: [
    ...
    '}',                                             // 6
    '',                                              // 7
    'void add(int value) {',                          // 8
    '    insert(count, value);',                      // 9
    '}',                                             // 10
    '',                                              // 11
    'void insert(int index, int value) {',            // 12
    '    if (index < 0 || index > count)',            // 13
    '        throw new IndexOutOfBoundsException();',  // 14
    '    for (int i = count; i > index; i--)',         // 15
    '        A[i] = A[i - 1];',                        // 16
    '    A[index] = value;',                           // 17
    '    count++;',                                    // 18
    '}',                                             // 19
    '',                                              // 20
    'void delete(int index) {',                       // 21
    '    if (index < 0 || index >= count)',           // 22
    '        throw new IndexOutOfBoundsException();',  // 23
    '    for (int i = index; i < count - 1; i++)',     // 24
    '        A[i] = A[i + 1];',                        // 25
    '    count--;',                                    // 26
    '}',                                             // 27
  ],
  cpp: [
    ...
    '}',                                             // 20
    '',                                              // 21
    'void remove(int index) {',                       // 22
    '    if (index < 0 || index >= count) {',         // 23
    '        cerr << "invalid index"; return;',       // 24
    '    }',                                          // 25
    '    for (int i = index; i < count - 1; i++)',     // 26
    '        A[i] = A[i + 1];',                        // 27
    '    count--;',                                    // 28
    '}',                                             // 29
  ],
...
const L = {
  ...
  addCall:   { pseudo: 8,  java: 9,  cpp: 9  },
  insCheck:  { pseudo: 11, java: 13, cpp: 13 },
  insErr:    { pseudo: 12, java: 14, cpp: 14 },
  insFor:    { pseudo: 13, java: 15, cpp: 16 },
  insShift:  { pseudo: 14, java: 16, cpp: 17 },
  insWrite:  { pseudo: 15, java: 17, cpp: 18 },
  insCount:  { pseudo: 16, java: 18, cpp: 19 },
  delCheck:  { pseudo: 19, java: 22, cpp: 23 },
  delFor:    { pseudo: 21, java: 24, cpp: 26 },
  delShift:  { pseudo: 22, java: 25, cpp: 27 },
  delCount:  { pseudo: 23, java: 26, cpp: 28 },
};
...
const mainF = ()                 => ({ fn: 'main',   vars: [] });
const addF  = (value)            => ({ fn: 'ADD',    vars: [vv('value', value)] });
 ...
 * is the call stack, bottom-to-top; the top (last) frame is the running one. */
...
function makeTrace() {
  return function* () {
    ...
    yield snap({
      ...
      note: ADD_NOTE,
      arr, count, frames: [mainF(), addF(40)],
    });
    ...
    yield snap({
      ...
      arr, count, frames: [mainF(), addF(40), insF(3, 40)],
    });
    yield snap({
    ...
    yield snap({
      ...
      arr, count, active: 3, frames: [mainF(), addF(40), insF(3, 40)],
    });
    count = 4;
    yield snap({
      line: L.insCount, tag: 'count',
      narrate: 'count = 4. Update the count — the list now holds four elements, indices 0–3.',
      arr, count, countActive: true, frames: [mainF(), addF(40), insF(3, 40)],
    });
    ...
    yield snap({
      ...
      arr, count, frames: [mainF(), insF(1, 25)],
    });
    yield snap({
      line: L.insFor, tag: 'loop',
    ...
    yield snap({
      ...
      arr, count, active: 4, frames: [mainF(), insF(1, 25, iVar(4, 'active'))],
    });
    arr[3] = arr[2];   // 30
    yield snap({
      line: L.insShift, tag: 'shift',
      narrate: 'i = 3. A[3] = A[2] = 30.',
      arr, count, active: 3, frames: [mainF(), insF(1, 25, iVar(3, 'active'))],
    });
    arr[2] = arr[1];   // 20
    yield snap({
      line: L.insShift, tag: 'shift',
      narrate: 'i = 2. A[2] = A[1] = 20. Index 1 is now safe to overwrite — its old value is already copied to index 2.',
      arr, count, active: 2, frames: [mainF(), insF(1, 25, iVar(2, 'active'))],
    });
    arr[1] = 25;
    yield snap({
      line: L.insWrite, tag: 'write',
      narrate: 'The loop is finished. A[1] = 25 — write the new value into the room just made.',
      arr, count, active: 1, frames: [mainF(), insF(1, 25, iVar(1, 'done'))],
    });
    count = 5;
    yield snap({
      line: L.insCount, tag: 'count',
      narrate: 'count = 5. Update the count — the list is now full: count = MAX = 5.',
      arr, count, countActive: true, frames: [mainF(), insF(1, 25, iVar(1, 'done'))],
    });
    ...
    yield snap({
      ...
      note: INVALID_NOTE,
      arr, count, phantom: holes, frames: [mainF(), insF(7, 99)],
    });
    yield snap({
    ...
    yield snap({
      ...
      arr, count, active: 1, frames: [mainF(), delF(1, iVar(1, 'active'))],
    });
    arr[2] = arr[3];   // 30
    yield snap({
      line: L.delShift, tag: 'shift',
      narrate: 'i = 2. A[2] = A[3] = 30.',
      arr, count, active: 2, frames: [mainF(), delF(1, iVar(2, 'active'))],
    });
    arr[3] = arr[4];   // 40
    yield snap({
      line: L.delShift, tag: 'shift',
      narrate: 'i = 3. A[3] = A[4] = 40. The elements are packed contiguously again.',
      arr, count, active: 3, frames: [mainF(), delF(1, iVar(3, 'active'))],
    });
    count = 4;
    ...
    yield snap({
      ...
      note: PAYOFF_NOTE,
      arr, count, countActive: true, frames: [mainF(), delF(1, iVar(4, 'done'))],
    });
  };
}
